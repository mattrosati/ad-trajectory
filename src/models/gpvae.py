...
class GPVAE(nn.Module):
    ...
    def compute_loss(self, x, t):
        B, T, D = x.shape
        x_flat = x.view(B * T, D)
        recon, mu, logvar, kl_gp = self(x_flat, t)

        recon_loss = ((recon - x_flat) ** 2).mean()
        kl_std = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) / (B * T)
        loss = recon_loss + kl_std + kl_gp
        return {"loss": loss, "recon_loss": recon_loss, "kl_std": kl_std, "kl_gp": kl_gp}