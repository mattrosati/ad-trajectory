    ...
    for f in os.listdir(args.load_dir):
        if f.endswith(".csv"):
            ...
            for t in tqdm(targets):
                ...
                if "train" in f:
                    v = embedding.get_embeddings(X_train = x.to_numpy(), y_train = y[t].to_numpy(), X = None, data_source="train")
                else:
                    ...
                    train_data, _ = load_data_for_tabPFN(os.path.join(args.load_dir, train_set))
                    train_x = train_data.drop(columns=targets)
                    train_y = train_data[targets]
                    v = embedding.get_embeddings(X_train=train_x.to_numpy(), y_train=train_y[t].to_numpy(), X = x.to_numpy(), data_source="test")
